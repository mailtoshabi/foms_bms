        ...
        <div class="col-xl-8 col-lg-10 col-12 mx-auto">
            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-primary text-white py-3">
                    <h5 class="card-title text-white mb-0"><i class="fas fa-plus-circle me-2"></i>Generate Manual Fee</h5>
                </div>
                <div class="card-body p-4">
                    <form method="POST" action="{{ route('admin.fees.store') }}">
                        ...
                        <div class="mb-4">
                            <label for="student_id" class="form-label fw-bold">Student <span
                                    class="text-danger">*</span></label>
                            <select name="student_id" id="student_id"
                                class="form-control @error('student_id') is-invalid @enderror" required></select>
                            @error('student_id')
                        ...
                        <div class="mb-4">
                            <label for="class_room_id" class="form-label fw-bold">Class Room <span
                                    class="text-danger">*</span></label>
                            <select name="class_room_id" id="class_room_id"
                                class="form-control @error('class_room_id') is-invalid @enderror" required></select>
                            @error('class_room_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-4">
                                <label for="type" class="form-label fw-bold">Fee Type <span
                                        class="text-danger">*</span></label>
                                <select name="type" id="type"
                                    ...
                                    <option value="admission" {{ old('type') == 'admission' ? 'selected' : '' }}>Admission
                                    </option>
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-4">
                                <label for="date" class="form-label fw-bold">Fee Date <span
                                        class="text-danger">*</span></label>
                                <input type="date" name="date" id="date"
                                    class="form-control @error('date') is-invalid @enderror"
                                    value="{{ old('date', date('Y-m-d')) }}" required>
                                <small class="text-muted">Due date will be automatically set to 7 days after this
                                    date.</small>
                                @error('date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
...
@section('script')
    ...
    <script>
        $(document).ready(function () {
            ...
            function toggleClassroomState() {
                var studentId = $('#student_id').val();
                $('#class_room_id').prop('disabled', !studentId);
            }

            // Initialize classroom state on load
            toggleClassroomState();

            // Clear and toggle classroom when student changes
