...
<button class="btn btn-sm btn-warning editSalary"
data-id="{{ $salary->id }}"
data-amount="{{ $salary->amount }}"
data-method="{{ $salary->payment_method }}"
data-date="{{ $salary->payment_date }}"
...
<form method="POST"
action="{{ route('staff.teacher-salaries.destroy',$salary->id) }}"
class="d-inline">
...
action="{{ route('staff.teacher-salaries.store',$teacher->id) }}">
...
$('#salaryForm').attr('action','/teacher-salaries/'+id);
